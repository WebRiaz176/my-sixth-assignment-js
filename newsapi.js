const loadCategory = async  () => {
    const fetchCategory = await fetch('https://openapi.programming-hero.com/api/news/categories');
    const allCategory = await fetchCategory.json();
    for(category of allCategory.data.news_category){
        console.log(allCategory.data.news_category.indexOf(category))
        const createItem = document.createElement('li');
        createItem.className = 'nav-item';
        createItem.innerHTML =  `
        <a class="nav-link category-nav-item" category="0${allCategory.data.news_category.indexOf(category)+1}" href="#">${category.category_name}</a>
        `;
...
function findNews () {    
        document.addEventListener('click', function(e){
         const index = e.target.getAttribute('category')
           fetch('https://openapi.programming-hero.com/api/news/category/${index}')
           .then(res => res.json())
           .then(data => {
            data.data.sort((a,b) => {
            ...
            // showNewsCategory(e.target.innerText)
